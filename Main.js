		...
		// Hê nhu bây bi iu iu chiu chiu
		function init() {
			canvas = document.getElementById("Meerucanvas");
			stage = new createjs.Stage(document.getElementById("Meerucanvas"));
			center_x = stage.canvas.width / 2;
			center_y = 0;

			font = 22 + "px " + "Cantarell";;
			text_field_1 = new createjs.Text("Click vào bông hồng nhaa", font, "#515151");
			...
			for (var i = count_i; i >= 1; i--) {
				...
				if (i == count_i) {
					tween = new createjs.Tween.get(shape, {
							loop: false
						})
						.to({
							scaleX: 13 * (i / 10),
							scaleY: 13 * (i / 10),
							rotation: (100 * i) % 360
						}, 5000 + i * 200, createjs.Ease.quintOut)
						.wait(1000);
			...
			function handleLoad(evt) {
				...
				if (createjs.Touch.enable(stage)) {} else {}
